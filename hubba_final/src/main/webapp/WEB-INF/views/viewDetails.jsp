...
function showsched(){
	...
	<hr>
	<div class="container-fluid" id="write_review_container"
		...
		<div class="col" style="padding-left: 15px; padding-right: 15px;">
			<div
				style="padding-left: 15px; padding-right: 15px; border-bottom: 1px solid gray; margin-bottom: 20px; font-size: 30px;">평가하기</div>
			<div class="inputAllBox"
				...
				<div class="row"
					...
					id="write_review_row">
					<div class="col-2"></div>
					<form method="post" id="insertReviewForm"
						enctype="multipart/form-data">
						<input type="hidden" name="user_idx" id="u_idx"
							value="${user_idx }" /> <input type="hidden" name="business_idx"
							id="b_idx" value="${business_idx }" />
						...
						<div class="insertImages">
							<div class="input_wrap">
								<label for="multiparts">업로드</label> <input type="file"
									name="multiparts" id="multiparts" multiple="multiple" />
							</div>
		...
		<div class="reviewContainer">
			<div class="allCount"
				style="margin: 20px 0px; border-bottom: 1px solid gray">${review_count }
				Reviews</div>
			<div class="reviewSection" id="reviewContent">
				<c:choose>
					<c:when test="${review_count eq 0 }">
						<div id="newRecordImg">
							<img src="resources/img/newRecord2.jpg"
					...
					<c:otherwise>
						<c:forEach var="review" items="${review }" varStatus="i">
							...
							<input type="hidden" name="review_uidx"
								value="${review.user_idx }" />
							<div class="tableDiv" id="${i.index }">
								<table class="reviewTable" id="example"
									style="width: 100%; margin-bottom: 30px;">
									<tr>
										<td rowspan="2" style="width: 10%"><c:choose>
												<c:when test="${review.name eq 'sns_login'}">
													<img
														style="width: 60px; height: 60px; border-radius: 50px; margin: 0 20% 0 20%;"
														src="${review.profile_img }">
												</c:when>
												<c:otherwise>
													<img
														style="width: 60px; height: 60px; border-radius: 50px; margin: 0 20% 0 20%;"
														src="resources/img/${review.profile_img }">
												</c:otherwise>
											</c:choose></td>

										<td style="font-size: 18px; padding-left: 10px; width: 70%"><p
									...
									<tr>
										...
										<td colspan="2" style="padding-left: 8px;"><span
											style="margin-bottom: 14px; margin-top: 8px;"><c:forEach
													begin="1" end="${review.rating }">

													<img src="resources/img/oneStar.png" style="width: 20px;" />
												</c:forEach></span></td>
									...
									<tr>
										<td></td>
										<td colspan="2"><c:if test="${review.images ne null }">
												<span style="margin-left: 10px;"> <c:forEach
														items="${fn: split(review.images, '&') }" var="images">
														<img
															style="width: 100px; height: 100px; margin-right: 5px;"
															src="resources/img/${images }" id="${images }"
															class="imgModal" />
									...
									<c:if test="${user_idx ne review.user_idx }">
										<tr>
											<td colspan="2" style="text-align: end;"
												id="reviewCount[${review.idx }]">${review.total_likes }</td>
											<td style="text-align: center;"><c:if
													test="${review.likes eq 1 }">
													<a href="javascript:void(0);" id="${review.idx }"
														style="text-align: left;" class="likesBtn done">♥</a>
													<span style="color: gray; margin-left: 10px; width: 25%;">도움이
														됐어요!</span>
													<input type="hidden" name="likes[${review.idx}]"
														id="likesValue" value="${review.likes }" />
												</c:if> <c:if test="${review.likes eq 0 }">
													<a href="javascript:void(0);" id="${review.idx }"
